...
export default function PhoneNumbersPage() {
    ...
    useEffect(() => {
        const loadNumbers = async () => {
            ...
            if (user) {
                const { data: clinic } = await supabase.from("clinics").select("id, subscription_status").eq("user_id", user.id).single();
                if (clinic) {
                                ...
                                <div className="flex justify-end mt-4 gap-3">
                                    ...
                                    {!hasSubscription ? (
                                        ...
                                        >
                                            ...
                                            {status === "loading" ? "Configuring SIP..." : (existingNumbers.length === 0 ? "✨ Get 1st Number FREE" : "Buy ($10 from Wallet)")}
                                        </button>
                                    )}
                                </div>
                            </motion.div>
